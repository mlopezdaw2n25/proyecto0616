                ...
                <aside class="lg:col-span-3">
                    <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-5">
                        <div class="flex flex-col items-center text-center gap-3">
                            <img src="{{ $usuaripost->avatar ?? 'https://i.pravatar.cc/120?u=' . ($usuaripost->id ?? 'default') }}" alt="Avatar {{ $usuaripost->name ?? 'Usuario' }}" class="w-24 h-24 rounded-full object-cover border-2 border-blue-500">
                ...
                <main class="lg:col-span-6 space-y-5">
                    <article class="bg-white rounded-2xl shadow-md border border-gray-200 p-5">
                        <header class="flex items-start gap-3">
                            <img src="{{ $usuaripost->avatar ?? 'https://i.pravatar.cc/40?u=' . ($usuaripost->id ?? 'default') }}" alt="Avatar" class="w-11 h-11 rounded-full">
                            <div>
                            ...
                            @if($usuari->tipus_user_id == $recomendado->tipus_user_id && $recomendado->id !== $usuari->id)
                                <article class="min-w-[110px] flex flex-col items-center text-center bg-gray-50 rounded-xl border border-gray-200 p-3">
                                    <a href="/perfiles/{{ $recomendado->id }}">
                                        <img src="{{ $recomendado->avatar ?? 'https://i.pravatar.cc/60?u=' . $recomendado->id }}" alt="{{ $recomendado->name ?? 'Perfil' }}" class="w-14 h-14 rounded-full object-cover">
                                    </a>
